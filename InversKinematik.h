#define coxaLength 24 // Panjang Coxa dalam milimeter 25
#define fermurLength 59 // Panjang Fermur dalam milimeter 50
#define thibiaLength 68 // Panjang Tibia dalam milimeter 60
...
vec3_t InversKinematik(vec3_t target){
    ...
    float b = thibiaLength - target.y;
     float thetaF2 = acos((pow(fermurLength, 2) + pow(a, 2) - pow(thibiaLength, 2) ) / (2 * a * fermurLength));
    // float thetaF = acos((pow(fermurLength, 2) + pow(x0, 2) - pow(b, 2) ) / (2 * x0 * fermurLength))* 180/PI;
